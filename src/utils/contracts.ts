...
import { AptosResourceType, AptosStructTag, CoinInfoAddress, CoinStoreAddress } from '../types/aptos'
...
export function getCoinStoreTypeFromArg(coinTypeArg: string) {
  return `${CoinStoreAddress}<${coinTypeArg}>`
}

export function getCoinInfoTypeFromArg(coinTypeArg: string) {
  return `${CoinInfoAddress}<${coinTypeArg}>`
}
